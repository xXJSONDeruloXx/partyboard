...
void InitializeDol()
{
    ...
    if (!dol || dolSize <= 0)
        return;
    destination = OSRoundUp32B((uintptr_t)OSGetArenaLo());
    if (destination + (uintptr_t)dolSize > (uintptr_t)OSGetArenaHi()) {
        OSReport("[PM/DOL] main.dol does not fit in the low arena\n");
        return;
    }
    dolPtr = reinterpret_cast<u8 *>(destination);
    memcpy(dolPtr, dol, dolSize);
    dolDataSize = dolSize;
    OSSetArenaLo(reinterpret_cast<void *>(OSRoundUp32B(destination + dolSize)));
}
...
void *GetDolIncludeData(int index)
{
    ...
    uint32_t address = entry.addresses[1];
    uint32_t offset = address - DOL_SHIFT;
    if (address == 0 || dolPtr == nullptr || offset + entry.size > static_cast<uint32_t>(dolDataSize)) {
        OSReport("Requested non-existing data\n");
        return nullptr;
    }
    void *ret = &dolPtr[offset];
    return ret;
}
